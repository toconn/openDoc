...
public class IntegerList implements Cloneable, java.io.Serializable, Iterable <Integer> {
						            
	private static final long 	serialVersionUID = 3266314499844219017L;

	public static String		CLASS_NAME		= IntegerList.class.getName();
	 ...
	 */
	private transient int[]	elementArray;
	...
	public IntegerList (int intArray[]) {
	...
	public void add(int intArray[]) {
		...
		if (intArray != null) {
			...
			ensureCapacity (size + intArray.length);
		
			for (i = 0; i < intArray.length; i ++) {
				
				elementArray[size] = intArray[i];
				size ++;
			}
		}
	}
	 ...
	 * @param newStringList tadhg.util.StringList
	 */
	public void add(IntegerList newList) {
	...
	public Object clone() {
		...
		try {
		    ...
		    newList = (IntegerList) super.clone();
		    
		    newList.elementArray = new int[size];
		    newList.size = size;
	...
	public void ensureCapacity (int minCapacity) {
		...
		if (minCapacity > oldCapacity) {
		    ...
		    elementArray = new int[newCapacity];
	...
	private void init(int ipInitialCapacity) {
		...
		elementArray	= new int[ipInitialCapacity];
		size		= 0;
	...
	/**
	 * Reconstitute the <tt>IntegerList</tt> instance from a stream (that is,
	 * deserialize it).
	 */
	private synchronized void readObject(java.io.ObjectInputStream s)
		...
		elementArray = new int[arrayLength];
	
		// Read in all elements in the proper order.
		for (int i=0; i < size; i++) {
	
			elementArray[i] = s.readInt();
		}
	...
	public void remove (int index) {
		...
		size --;
		elementArray[size] = 0; // Let gc do its work
	...
	public java.util.Iterator <Integer> iterator() {
		...
		iterator = new UaIterator <Integer> (size);
		
		for (index = 0; index < size; index++) {
			
			iterator.add(new Integer (elementArray [index]));
		}
