...
public class OWebApp implements IApp {
	...
	public void displayData (List <StringList>	aliasStringListList) {
		...
		displayStringList.add ("Website List");
	...
	public void init()  {
		...
		AppStore.appInfo.setProperty (IAppConst.APP_NAME,			IOWebConst.APP_NAME);
		...
		AppStore.paramDefPack				= IOWebConst.PARAM_DEF_PACK;
		...
		try {
			...
			// Init document aliases (load properties)...
			
			try {
				
				this.oDirComponent = new oDirComponent (IODirComponentConst.ODIR_TYPE_WEB);
